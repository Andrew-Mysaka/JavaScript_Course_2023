  ...
  for (const [i, name] of names.entries()) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if(i === 0){
        ...
        word += w.replace(w[0], w[0].toUpperCase())
      }
    }
...
console.log(new Set('andriimysaka').size);
*/
...
for "draw"). HINT: Note how the odds and the game objects have the same property names 😉
...
for (const [i, player] of score.entries()) {
...
for (const odd of odds) summa += odd;

average = summa / odds.length;
console.log(average);
