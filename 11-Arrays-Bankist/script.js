...
console.log("___________FOREACH________________");
...
const checkDogs = function(dogsJulia, dogsKate){
  ...
  allDogs.forEach(function(dog, i){
    const isPuppy = dog >= 3 ? `an adult, and is ${dog} years old` : "still a puppy 🐶";
...
const calcAverageHumanAge = function(ages){
  ...
  const averageAdultDogsYears = adultDogs.reduce((sum, cur) => sum + cur, 0) / adultDogs.length;

  return averageAdultDogsYears;
  // console.log(agesInHumanYears);
  // console.log(adultDogs);
  // console.log(averageAdultDogsYears);
}
...
const randArray = Array.from({length: 100}, () => Math.ceil(Math.random() * 6));
...
are eating too much or too little.Eating too much means the dog's current food
...
create a new array, simply loop over the array. Formula: recommendedFood = weight ** 0.75 * 28.
...
and Alice and Bob's dogs eat too much!" and "Sarah and John and Michael's dogs eat too little!"
...
const dogEating = function(dog){
  console.log(`Dog eating too ${dog.curFood > dog.recommendedFood ? 'much' : 'little'}`);
}
...
// 4. Log a string to the console for each array created in 3., like this: "Matilda
// and Alice and Bob's dogs eat too much!" and "Sarah and John and Michael's dogs eat too little!"
const logDogsEat = function(owners, eatEnough = false){
  console.log(`${owners.join(" and ")}'s dogs eat too ${eatEnough ? 'much' : 'little'}!`);
}
logDogsEat(ownersEatTooMuch, true);
logDogsEat(ownersEatTooLittle);
